...
let evenNumbers= mixedNumbers.filter(function(val, i , arr){
  return val%2 ===0
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(val,i,arr){
  return arr[i].CP > 200;
  // console.log(arr[i].CP > 200) 
...
let orderTotals = orders.map(num => num.price * (num.tax + 1))

// let orderTotals = orders.map(function(val,i,arr){
 
//   return val.price * (val.price + val.tax)
// })
...
let bobsPurchases = purchases.filter(function(val,i,arr){
  if(val.owner === 'Bob'){return val}
})
// console.log(bobsPurchases)

let bobsTotal = bobsPurchases.reduce(function(total, element, index, array){
